...
# Additional role so the SP can create / update resource groups tagged for
# the runtime (the RG itself must already exist or the admin must create it
# before handing off).
_RG_READER_ROLE = "Reader"

# The runtime needs to read and write Key Vault secrets (e.g. MicrosoftAppId,
# MicrosoftAppPassword) that are required for bot channel registration.
_KV_SECRETS_ROLE = "Key Vault Secrets Officer"

# Required for interacting with ACA Dynamic Sessions (code interpreter /
# shell sandbox).  Must be scoped to the session pool resource.
    ...
    * **User-assigned managed identity** -- used in ACA deployments.  The
    ...
    def _session_pool_scope(self, subscription_id: str) -> str | None:
        ...
        try:
            ...
            if pool_id:
                return pool_id
            # Fall back to constructing the scope from individual fields
